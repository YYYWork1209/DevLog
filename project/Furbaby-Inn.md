...
## 缓存一致性问题（保证数据一致）
 最开始采用旁路缓存（Cache-Aside）先写后删（先删后写会导致写的过程中数据全部打到数据库上，所以先写后删）
缓存策略： Cache-Aside 模式 — 先查缓存，命中直接返回；未命中则查数据库，回填缓存后返回。**更新时先写数据库，再删除对应缓存**。

## 更新时用“删除缓存”而不是“更新缓存”
### 主要是资源损耗问题
更新（写）：每次数据库Update，都要执行一次Redis的SET操作。如果该数据1分钟被改1000次，但1小时才被读1次，那这1000次SET全是无效写，浪费了大量Redis带宽和CPU。

删除：每次数据库Update，只执行一次Redis的DEL操作（极轻量）。只有真正被“读”的时候，才执行一次SET。懒加载（Lazy Loading） 极大节省了资源。
### 脏数据好不好处理的问题

当数据库事务是回滚：
- 如果采用“先写库，再更新缓存”，但数据库事务提交失败（回滚）了。此时，缓存里已经被写入了那个“压根不存在于数据库”的新值。这是一个“幽灵数据”，只要TTL不过期，它会永久污染缓存。
- 如果采用“先写库，再删除缓存”，即使数据库事务回滚了，我们最多只是多删了一次缓存（删了个寂寞）。缓存里没有写入任何虚假数据，下一次读请求去查库，拿到的依然是正确的旧数据。
---

## 来看当缓存中没有数据的情况

如果更新时去“写”缓存而不是删除，在高并发下会引发严重的脏数据问题。

首先要明确一个前提：线程A之所以要去“读旧数据”，是因为它发现缓存未命中，所以它必须去数据库捞数据准备回填。

在这个前提下，看“更新时直接写缓存”（而不是删除）会发生什么：

时间线推演（脏数据诞生过程）

T1 时刻（读请求）：线程A发起读请求，缓存里**没有数据（或已过期）**。于是线程A去数据库查询，成功拿到了旧数据 V1。此时，线程A刚执行完 select，还没来得及执行 redis.set(V1)（可能正在做序列化，或者网络轻微卡顿）。

T2 时刻（写请求）：就在这个微小的间隙里，线程B发起写请求，将数据库中的 V1 更新为了 V2，并且因为策略是“更新时写缓存”，线程B顺手就把 V2 写进了缓存。

T3 时刻（回填）：此时，T1时刻的线程A从网络卡顿中恢复过来，执行 redis.set(V1)。因为线程A根本不知道数据库已经变了，它只知道要把自己手里这份“旧数据”塞进缓存。

最终结果：数据库里是新的 V2，而缓存里是被线程A强行覆盖回去的旧 V1。

同理其实先写后删也解决不了，当线程B写完之后去删除完缓存，线程A把旧值写入了，这样后续读请求会一直读到旧值，脏数据仍然存在！

## 解决脏数据方案
### 1.延迟双删（RabbitMQ）
既然无法阻止线程A在T3时刻写入旧值，那我们就在写操作完成后，等一会儿，再删一次。

- 流程：线程B写数据库 -> 删除缓存 -> sleep(50ms) -> 再次删除缓存。

- 原理：sleep的时间必须大于线程A“查库 + 回填缓存”所消耗的时间。等线程A把旧数据写回去之后，线程B恰好醒来，执行第二次删除，精准把刚回填的旧值清掉。

- 缺点：sleep时间很难精确控制（网络抖动可能导致回填变慢），且sleep期间该key在缓存中确实是空的，会引发短暂的缓存穿透。

### 2.互斥锁（读写锁/分布式锁）
既然“查询数据库+回填缓存”这个组合动作不是原子的，那我们就给这个动作加锁，让“读回填”和“写删除”串行化。

- 写请求（线程B）：获取写锁 -> 写数据库 -> 删除缓存 -> 释放写锁。

- 读请求（线程A）：缓存未命中 -> 获取读锁（或互斥锁） -> 查数据库 -> 回填缓存 -> 释放读锁。

- 效果：如果线程A正在回填旧数据（持有锁），线程B的写操作必须等待A释放锁，B绝不会在A回填期间去删缓存（因为还没轮到B执行）。反过来，如果B正在写库删缓存，A必须等B释放锁后才能去查库，A拿到的就是最新数据。彻底从根源上杜绝了并发覆盖（性能降低）。

### 依据业务情况进行选择
- 强一致性：互斥锁、读写锁
- 高可用性：延迟双删（加锁期间写操作被阻塞，性能降低）
