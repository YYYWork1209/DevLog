...
| 配置项 | 值 | 说明 |
|--------|-----|------|
| 默认首页 | `index.html` | 访问根路径时返回的页面 |
| 默认 404 页 | `index.html` | 页面找不到时回退到首页 |
| 子目录首页 | ✅ **开通** | 访问子目录时自动返回下面的默认首页 |
| 文件 404 规则 | **Index** | 子目录不存在时直接返回主页内容，不跳转 URL |
...
```
访问 /java/JavaSE 时：
  开通 → 自动返回 /java/JavaSE/index.html ✅（VitePress cleanUrls 依赖此机制）
  不开 → 返回 403，因为 OSS 不列出目录内容
```

#### 文件 404 规则为什么选 Index？

| 选项 | 访问 `/java/JavaSE` 时的行为 | URL 变化 |
|------|---------------------------|----------|
| Redirect | 302 跳转到 `/java/JavaSE/` | 地址栏会变，多一个 `/` |
| NoSuchKey | 直接 404 | 页面报错 |
| **Index** ✅ | 直接返回 `/java/JavaSE/index.html` 内容 | **URL 干净不变** |

VitePress 配置了 `cleanUrls: true`，靠 Index 模式实现无 `/` 结尾且不跳转。Redirect 会在 URL 后拼接 `/`，破坏干净的 URL 体验。
...
## 五、临时访问：GitHub Pages（备案通过前）

### 为什么 OSS 默认域名会强制下载？

部署到 OSS 后，通过默认域名（`https://xxx.oss-cn-hangzhou.aliyuncs.com`）访问时，会发现浏览器强制下载文件而不是渲染页面。查看响应头：
...
这是**阿里云 OSS 的安全策略**，2022 年 10 月后新开通 OSS 的用户，通过默认域名访问任何文件都会被强制添加下载头。**这个行为无法关闭**，官方唯一解是绑定自定义域名。

但域名备案通常需要 1-2 周，等待期间可以用 **GitHub Pages** 作为临时访问入口。

### 启用 GitHub Pages

部署流水线已经配置好了，每次推送会自动部署到 `gh-pages` 分支。你只需要在 GitHub 仓库手动打开开关：
...
等几十秒，访问地址：
```
https://yyywork1209.github.io/DevLog/
```

### 备案通过后

绑定自定义域名到 OSS（见[下一章](#六绑定自定义域名备案通过后)），GitHub Pages 可以保留作为备用入口或直接关闭。
